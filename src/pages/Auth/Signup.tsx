...
export default function SignUpPage() {
  ...
  const handleSubmit = async () => {
  // 1. Validate before setting loading state
  if (!form.name.trim() || !form.email.trim() || !form.password.trim()) {
    alert("Please fill in full name, email, and password.");
    return; // Loading is still false here, so no UI freeze
  }
  ...
  try {
    // 2. Use the correct Supabase Auth method
    const { data, error } = await supabase.auth.signUp({
      email: form.email.trim(),
      password: form.password,
      options: {
        // This stores the name in the user's metadata
        data: {
    ...
    // 3. This ensures loading is ALWAYS set to false, even if it fails
    setLoading(false);
  }
};
      ...
      <main className="flex-1 flex items-center justify-center py-12 px-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 w-full max-w-sm p-8">
          ...
          {/* <p className="text-center text-xs text-gray-500 mt-5">
            Already have an account? <button onClick={() => navigate('/login')} className="ml-1 text-blue-500 font-semibold hover:underline">LogIn</button>
        </p> */}
        </div>
      </main>
